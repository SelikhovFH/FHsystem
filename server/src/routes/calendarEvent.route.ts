...
class CalendarEventsRoute implements Routes {
  ...
  private initializeRoutes() {
    ...
    this.router.post(`${this.path}`, isEditorMiddleware,
      validationMiddleware(CreateCalendarEventDto, 'body'), this.calendarEventController.createCalendarEvent);
    this.router.delete(`${this.path}/:id`, isEditorMiddleware, validationMiddleware(DeleteDto, 'params'), this.calendarEventController.deleteCalendarEvent)
    this.router.patch(`${this.path}`,
      isEditorMiddleware, validationMiddleware(UpdateCalendarEventDto, 'body'),
